...
Production defaults dùng canonical plural model pools:
...
| xKiro `401/402` | disable credential scope; rotate target nếu còn sibling hợp lệ |
...
Legacy scalar provider model/key fields remain migration/backward-compatibility inputs only when the corresponding plural field is absent. Do not set only scalars on a fresh synced `.env` because blank plural fields are explicit and intentionally win.
...
Concrete target identity là secret-free `(family, route, model, credential_id)` với opaque target ID. `credential_id`/`target_id` không bao giờ chứa raw API key. Chainnode có ordered model targets; xKiro có model-major × credential targets.
...
- migrate legacy scalar provider values sang canonical plural pools khi plural field chưa tồn tại;
...
python -m unittest tests.test_provider_pool_config tests.test_provider_pool_migration -v
...
6. `/status`;
7. controlled Chainnode failure -> xKiro fallback sau khi xKiro đã live-qualified/configured bằng plural pools;
8. controlled xKiro resource failure -> scoped sibling rotation without duplicate tool execution;
