...
def _parse_values(text: str, *, source: str) -> dict[str, str]:
    values: dict[str, str] = {}
    for line in text.splitlines():
        stripped = line.strip()
        if not stripped or stripped.startswith("#"):
            continue
        key, value = _split_assignment(line, source)
        if key in values:
...
def _sync() -> None:
    ...
    example_values = _parse_values(example, source=".env.example")
    current_values = _parse_values(current, source=".env")
    unknown_keys = sorted(set(current_values) - set(example_values))
    if unknown_keys:
        raise ValueError(f"unknown key(s) in .env: {', '.join(unknown_keys)}")
