...
class QuestionProcessor:
    ...
    async def execute(self, record, operations):
        ...
        try:
            ...
            # The outer JobManager boundary records sanitized identity + exception type.
            # Do not retain or log raw provider/tool exception text here: upstream
            # messages can contain prompts, response bodies, URLs, or other payloads.
            self.stats.record_error(type(exc).__name__)
    ...
    async def deliver(self, record, answer) -> None:
        ...
        self.memory.push_exchange(
            ...
            order_key=record.submission.request_message_id,
        )
