...
class JobManager:
    ...
    @staticmethod
    def _failure_location(exc: Exception) -> str:
    ...
    async def _run(self, record: JobRecord) -> None:
        try:
            ...
            logger.error(
                "Unexpected controlled job failure job_id=%s state=%s "
                "exc_type=%s location=%s",
                record.job_id,
                record.control.state,
                type(exc).__name__,
                self._failure_location(exc),
            )
    ...
    @staticmethod
    def _prune_terminal_payload(record: JobRecord) -> None:
    ...
    async def _remember_terminal(self, record: JobRecord) -> None:
        async with self._lock:
            if record.control.state not in TERMINAL:
                return
            self._prune_terminal_payload(record)
