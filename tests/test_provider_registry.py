...
class ProviderContractTests(unittest.TestCase):
    ...
    def test_aurora_family_is_registered_text_only_and_uses_temporary_auth_policy(self) -> None:
        self.assertIn("aurora", PROVIDER_FACTORIES)
        slots = PROVIDER_FACTORIES["aurora"](
            Settings(_env_file=None, aurora_api_key="internal-secret")
        )
        try:
            self.assertEqual([slot.name for slot in slots], ["aurora"])
            self.assertEqual(slots[0].capabilities.route, "text")
            self.assertFalse(slots[0].capabilities.supports_vision)
            self.assertEqual(slots[0].capabilities.max_images, 0)
            self.assertEqual(slots[0].health.auth_failure_cooldown_sec, 60.0)
        finally:
            asyncio.run(_close_slots(slots))


class ProviderOrderTests(unittest.TestCase):
    def test_provider_orders_default_to_bai_then_aurora_for_text_only(self) -> None:
    ...
    def test_production_router_keeps_bai_first_and_skips_unconfigured_aurora(self) -> None:
        self.assertIn("aurora", PROVIDER_FACTORIES)
        router = build_provider_router(
            Settings(_env_file=None, bai_api_key="secret")
        )
        try:
            self.assertEqual(router.provider_order(False), ("bai", "aurora"))
    ...
    def test_production_router_adds_aurora_when_service_key_is_configured(self) -> None:
        self.assertIn("aurora", PROVIDER_FACTORIES)
        router = build_provider_router(
            Settings(
                _env_file=None,
        ...
        try:
            self.assertEqual(router.configured_provider_names(False), ("bai", "aurora"))
            self.assertEqual(router.configured_provider_names(True), ("bai",))
            self.assertEqual(
